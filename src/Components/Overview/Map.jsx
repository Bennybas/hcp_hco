...
const USAMap = ({ onStateSelect }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true)
        const response = await fetch("http://127.0.0.1:5000/fetch-map-data")
        if (!response.ok) {
      ...
      if (hcoId && hcoState && hcoId !== "-" && hcoState !== "-") {
        ...
        if (hcoLat !== null && hcoLong !== null) {
          ...
          if (existingLocation) {
            ...
            // Add new location with the correct name
    ...
    setTimeout(() => {
      ...
      try {
        ...
        stateLocations.forEach((location) => {
          ...
          console.log("Location name:", location.name, "Location ID:", location.id)
      ...
      <div className="relative bg-white rounded-xl border border-gray-200 shadow-sm">
        ...
        <div className="absolute bottom-4 right-4 bg-white p-2 rounded-md shadow-md text-xs z-10">
          ...
          <div className="flex items-center mt-2">
            ...
            <span className="text-[10px]">HCO Location</span>
          </div>
        </div>
      </div>
